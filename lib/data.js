...
export function defineDatabase(path) {
  return new Promise((resolve) => {
    ...
    db.simulationsSourceIdx = sub(db, 'simulationsSourceIdx', { keyEncoding: charwise, valueEncoding: 'json' });
    instrumentDatabase(db.simulationsSourceIdx, 'simulationsSourceIdx');

    db.executions = sub(db, 'executions', { valueEncoding: 'json' });
    instrumentDatabase(db.executions, 'executions');

    db.executionsSimulationIdx = sub(db, 'executionsSimulationIdx', { keyEncoding: charwise, valueEncoding: 'json' });
    instrumentDatabase(db.executionsSimulationIdx, 'executionsSimulationIdx');

    db.renderings = sub(db, 'renderings', { valueEncoding: 'json' });
    instrumentDatabase(db.renderings, 'renderings');

    db.renderingsExecutionSimulationIdx = sub(db, 'renderingsExecutionSimulationIdx', { keyEncoding: charwise, valueEncoding: 'json' });
    instrumentDatabase(db.renderingsExecutionSimulationIdx, 'renderingsExecutionSimulationIdx');
