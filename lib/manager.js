...
class Manager {
  ...
  getSimulationsStream() {
    const query = { ...this.dbOptions };
    const simulationStream = this.db.simulations.createValueStream(query);
    return _(simulationStream)
      .consume(this.addExecutionsToSimulationStreamItem.bind(this));
  }

  getSimulationsForSourceStream(sourceId) {
    const query = {
      ...this.indexOptions,
      gte: [sourceId],
      lt: [sourceId, '\xff'],
    ...
    return _(simulationIndexStream).flatMap(getSimulationStream)
      .consume(this.addExecutionsToSimulationStreamItem.bind(this));
  }
  ...
  getSimulations(cb) {
    ...
    return cb
      ...
      : new Promise((resolve) => this.getSimulationsStream().toArray(resolve));
  }
  ...
  getSimulationsForSource(sourceId, cb) {
    ...
    return cb
      ...
      : new Promise((resolve) => this.getSimulationsForSourceStream(sourceId).toArray(resolve));
  }
  ...
  getExecutionsStream(simulationId) {
    ...
    return _(executionIndexStream).flatMap(getExecutionStream)
      .consume(this.addRenderingsToExecutionStreamItem.bind(this));
  }
