...
// const debug = _debug('lvl:abstract-leveldown');
  ...
  for (var i = 0; i < array.length; i++) {
    if (typeof array[i] !== 'object' || array[i] === null) {
      return process.nextTick(
        callback,
        new Error('batch(array) element must be an object and not `null`'),
      );
    }
...
AbstractLevelDOWN.prototype._setupIteratorOptions = function(options) {
  // debug('_setupIteratorOptions', { beforeClean: { ...options } });
  options = cleanRangeOptions(this, options);
  // debug('_setupIteratorOptions', { afterClean: { ...options } });
  ...
  // debug('_setupIteratorOptions', { augmented: { ...options } });
  return options;
};
