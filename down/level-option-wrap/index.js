...
module.exports = function(opts, prefix) {
  ...
  if (!prefix) prefix = {};
  const xopts = {};
  const gte = defined(prefix.gte, prefix.ge, prefix.start);
  const lte = defined(prefix.lte, prefix.le, prefix.end);
  const ogte = defined(opts.gte, opts.ge, opts.start);
  const olte = defined(opts.lte, opts.le, opts.end);
