...
function SubIterator(db, ite, prefix) {
  // debug('SubIterator:ctor', { db, ite, prefix });
  this.iterator = ite;
  this.prefix = prefix;
...
SubIterator.prototype._next = function(cb) {
  // debug('SubIterator:_next');
  const self = this;
  // eslint-disable-next-line no-unused-vars
  this.iterator.next(function(err, key, value) {
    // debug('SubIterator:_next:embeddedIterator.next cb', { err, key, value });
...
SubDown.prototype._open = function(opts, cb) {
  ...
  this.db.open(function(err) {
    ...
    const subdb = down(self.db, 'subleveldown');
...
function isAbstract(db) {
  ...
  return Object.keys(abstract.AbstractLevelDOWN.prototype)
